...
### Mathematical Model
- **State ($x$):** $\begin{bmatrix} p \\ v \end{bmatrix}$ (Position and Velocity)
- **State Transition ($F$):** $\begin{bmatrix} 1 & dt \\ 0 & 1 \end{bmatrix}$
- **Observation ($H$):** $\begin{bmatrix} 1 & 0 \end{bmatrix}$ (We only measure position, velocity is inferred)
- **Process Noise ($Q$):** $\begin{bmatrix} q_{pos} & 0 \\ 0 & q_{vel} \end{bmatrix}$
- **Measurement Noise ($R$):** $\begin{bmatrix} r_{pos} \end{bmatrix}$
